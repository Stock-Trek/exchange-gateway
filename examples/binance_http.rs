...
use exchange_types::binance::http::{
    BinanceHttpRequest, BinanceHttpResponse, BinanceHttpResponseResult, BinanceHttpUnsignedRequest,
};
use std::sync::Arc;
...
fn to_transport_request(request: BinanceHttpRequest) -> EGResult<HttpRequest> {
    let (method, query) = match request.params {
        ...
        BinanceHttpUnsignedRequest::SpotOrderRequest(_) => (reqwest::Method::POST, String::new()),
    };
    Ok(HttpRequest {
        method,
        query: Some(query),
...
fn to_external_response(response: BinanceHttpResponse) -> EGResult<String> {
    match response.result {
        Some(BinanceHttpResponseResult::ExchangeInfo(info)) => Ok(format!(
            "exchangeInfo: {} symbols, serverTime {}",
            info.symbols.len(),
            info.serverTime
        )),
        _ => Ok(format!("response: {response:?}")),
    }
}
