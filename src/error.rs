...
mod tests {
    ...
    fn external() -> EGError {
        EGError::external(std::io::Error::other("boom"))
    }

    fn http_error(status: u16) -> EGError {
        EGError::HttpError {
            status,
            body: Vec::new(),
        }
    ...
    #[test]
    fn retries_failures_that_may_be_safe_to_repeat() {
        assert!(EGError::send_not_sent(EGError::TimedOut).is_retryable());
        assert!(EGError::send_unknown(external()).is_retryable());
    }

    #[test]
    fn retries_only_transient_http_statuses() {
        assert!(EGError::send_failed(http_error(408)).is_retryable());
        assert!(EGError::send_failed(http_error(500)).is_retryable());
        assert!(!EGError::send_failed(http_error(400)).is_retryable());
    }

    #[test]
    fn does_not_retry_rate_limit_or_internal_failures() {
        assert!(!EGError::send_unknown(EGError::RateLimited).is_retryable());
        assert!(!EGError::send_unknown(EGError::MutexPoisoned).is_retryable());
        assert!(!EGError::send_not_sent(EGError::ClockNotSynced).is_retryable());
        assert!(!EGError::NotConnected.is_retryable());
    }
}
