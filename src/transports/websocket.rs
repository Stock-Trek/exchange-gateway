    ...
    EGRes: Send + Sync + 'static,
{
    ...
    async fn send_and_wait_for(
        ...
        let transport_req = self.try_convert_request(request)?;
        // Register the message handler *before* sending the message to the
        // client, so a response that arrives immediately after sending is not
        // missed while the transport waits for it.
        let waiter = self.websocket_listener.register_filtered_response(filter)?;
        self.client.send_message(transport_req, timeout).await?;
        waiter.await
    }
    async fn disconnect(&self) -> EGResult<()> {
